...
import { selectActiveSourceMeters } from '../../redux/api/metersApi';
...
export const selectVisibleMetersAndGroups = createSelector(
	selectActiveSourceMeters,
	selectAllGroups,
	selectIsAdmin,
	(meterDataByID, groupDataById, isAdmin) => {
		// Holds all meters visible to the user, filtered to active MQTT source
...
export const selectDisplayableMetersAndGroups = createSelector(
	selectActiveSourceMeters,
	selectAllGroups,
	(meterDataByID, groupDataById) => {
		const meters = new Set<number>();
		const groups = new Set<number>();
		meterDataByID.forEach(meter => {
